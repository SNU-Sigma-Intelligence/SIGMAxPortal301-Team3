...
def random_plane(min_dist, max_dist, max_angle):
    ...
    d = np.random.uniform(min_dist, max_dist)
    p0 = n * d
    return {'normal': n, 'point': p0}

# 여러 개의 평면 생성
planes = [random_plane(min_distance, max_distance, max_angle_rad) for _ in range(num_planes)]
...
plane_data.to_csv('plane_dataset.csv', index=False)